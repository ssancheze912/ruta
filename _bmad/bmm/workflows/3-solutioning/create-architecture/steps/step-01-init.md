...
#### A2. Load Company Standards (If Exist)

Check for company standards at `{company_standards_path}`:

- If directory exists, load ALL .md files from it
- Set `workflowMode` = "strict" if standards found, else "discovery"
- Report: "✅ Estándares corporativos cargados. Modo: ESTRICTO"
...
**UI Kit Validation (if frontend project):**

- Check `{project-root}/package.json` for "siesa-ui-kit"
- If NOT found: "⚠️ siesa-ui-kit no instalado. Ejecuta: npm i siesa-ui-kit"

**UI Kit Component Reference (if frontend project):**

- Fetch the siesa-ui-kit component catalog from: `https://siesa-ui-kit.pages.dev/llms.txt`
- This provides the complete inventory of available UI components (forms, navigation, display, POS, business, layout views)
- Use this catalog during architectural decisions to ensure component reuse over custom development
- Store the component list in memory for reference throughout the architecture workflow
- If the URL is unreachable, warn the user: "⚠️ No se pudo acceder al catálogo de componentes de siesa-ui-kit. Las decisiones de componentes se basarán en los estándares corporativos locales."
