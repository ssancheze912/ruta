...
|------|------------|---------|-------------------|---------------|
| Unitarias Backend | xUnit + Moq | Validators, Handlers, Domain | ≥ 80% | 100% |
| Unitarias Frontend | Vitest + RTL | Hooks, Stores, Componentes | ≥ 75% | 100% |
| Integración Backend | xUnit + TestContainers | Repos, Endpoints, EF Core | ≥ 70% | 100% |
| Integración Frontend | Vitest + MSW | API calls, TanStack Query | ≥ 60% | 100% |
| Contract Tests | Pact / OpenAPI schema | Contratos entre microservicios | 100% endpoints | 100% |
| Smoke Tests | xUnit + HttpClient | Endpoints críticos post-deploy | Todos los críticos | 100% |
| Regresión | Suite completa | Pre-release | 100% | 100% |
| Performance | k6 / NBomber | Endpoints críticos (POS, cache) | P95 < umbral | Semi-auto |
| Seguridad RBAC | xUnit | Permisos × endpoints | 100% endpoints | 100% |
| Seguridad SAST | CodeQL + Snyk | Código fuente + dependencias | 100% PRs | 100% |
| Exploratoria | Sesiones guiadas | Flujos complejos, edge cases | Features P1 y P2 | Manual |
...
**Usuarios de prueba requeridos** (solo los nuevos o que cambian):
| Rol | Permisos | Ambiente |
|-----|---------|---------|
| [Inferir de la feature] | | QA |
